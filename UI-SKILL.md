...
- **Mustaqil bloklar** (`public/js/bank-ledger.js`): biri yiqilsa, qolgani ishlaydi.
  ...
  - **"Bank hisoblari" kartasi** (`treasury VIEW`): Kompaniya (Global/UGS/UTAX) → Hisob (kompaniyaga qarab filtrlanadi) select'lari, manba badge'lari (`SRC_BANK_FILE`, `SRC_MANUAL`). **Davr — sahifa tepasidagi `dateRange`** (alohida oy tanlovi yo'q): oraliq bank ko'chirmalari qamroviga qirqiladi va izohda ko'rsatiladi; boshlang'ich qoldiq oraliq boshiga hisoblanadi; kassa (oylik yig'indi) faqat oraliqqa to'liq kirgan oylar bilan. Standart oraliq — mavjud ma'lumotning birinchi → oxirgi sanasi (`/api/dashboard?auto=1` → `auto_range`), tashqaridagi kunlar o'chiq (`rng.limit`). Kompaniya/hisob URL'da: `#/dashboard?company=ugs&account=<raqam>` (`history.replaceState`). `.kpis.c4`: Boshlang'ich qoldiq · Tushum · Xarajat · Balans (accent). Tushum/Xarajat kartasi `sub` da yalpi summani ko'rsatadi. Karta bosilsa drawer ochiladi: Tushum/Xarajat → operatsiyalar `dataTable`, Boshlang'ich/Balans → hisoblar kesimidagi tekshiruv jadvali.
