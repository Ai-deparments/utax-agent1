...
import { h, kpiCard, card, fmt, money, short, badge, date, dataTable, segmented, kv } from '../ui.js';
...
export default async function render(root, { setTitle }) {
  ...
  async function load() {
    const [f, all] = await Promise.all([get('/api/forecast?days=' + days), get('/api/forecast/all')]);
    const sc = f.scenarios;
    setTitle('Prognoz', `Pul oqimi prognozi · ${days} kun · ${date(f.as_of)} → ${date(f.to)}`);
  ...
  root.append(h('div', { class: 'mb16' }, segmented([[7, '7 kun'], [30, '30 kun'], [90, '90 kun'], [180, '6 oy'], [365, '12 oy']], days, (v) => { days = v; load(); })), body);
  await load();
}
