...
export default async function render(root, { setTitle, can }) {
  ...
  const body = h('div', {});
  const AD_ICON = { BANK_API: 'bank', GOOGLE_SHEETS: 'file', EXCEL: 'upload', ONE_C: 'layers', ERP: 'briefcase', TELEGRAM: 'send', EMAIL: 'inbox', GEMINI: 'sparkles', GROQ: 'zap', LEDGER: 'layers', WEBHOOK_IN: 'link' };
  async function load() {
  ...
  function schemaFields(a, values = {}, secrets = {}) {
    ...
    const sec = Object.entries(a.secret_schema || {}).map(([k]) => { const [label, hint] = FIELD_BY_TYPE[a.type]?.[k] || FIELD[k] || [k, '']; const req = k === 'bot_token' || (['GEMINI', 'GROQ'].includes(a.type) && k === 'api_key' && !secrets[k]); return { name: 'sec_' + k, label, type: 'password', value: secrets[k] || '', hint: (hint ? hint + ' · ' : '') + 'shifrlangan holda saqlanadi', full: k === 'bot_token' || ['GEMINI', 'GROQ'].includes(a.type), required: req }; });
