...
import { h, kpiCard, card, fmt, short, badge, periodPicker, dataTable, pct, monthLabel, alert, emptyState } from '../ui.js';
...
export default async function render(root, { setTitle, params }) {
  ...
  async function load() {
    if (tab === 'pnl') {
      ...
      const t = p.totals;
      setTitle('Foyda va zarar (P&L)', `Tan olingan daromad − xarajatlar · ${p.period.label}`);
      ...
      setTitle('Xizmatlar rentabelligi', `Har bir xizmat turi bo‘yicha daromad va foyda · ${sp.period.label}`);
      body.replaceChildren(
        ...
        h('div', { class: 'grid g2 mb16' }, card('Daromad va foyda', barChart({ labels: sp.rows.map((r) => r.code), series: [{ name: 'Daromad', values: sp.rows.map((r) => r.revenue) }, { name: 'Yalpi foyda', values: sp.rows.map((r) => r.gross_profit) }, { name: 'Sof foyda', values: sp.rows.map((r) => r.net_profit), color: 'var(--s3)' }] })), card('Sof marja, %', barChart({ labels: sp.rows.map((r) => r.code), series: [{ name: 'Sof marja', values: sp.rows.map((r) => r.margin) }], money: false, yFmt: (v) => v + '%' }))),
