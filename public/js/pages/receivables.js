...
export default async function render(root, { setTitle, can, navigate }) {
  ...
  const cols = [
    { key: 'client', label: 'Mijoz', render: (r) => h('div', {}, h('b', {}, r.client), h('div', { class: 'xs muted' }, `INN ${r.inn || '--'} · ${r.manager || ''}`)) }, { key: 'contract_number', label: 'Shartnoma', render: (r) => h('a', { href: '#/contracts/' + r.contract_id, onClick: (e) => e.stopPropagation() }, r.contract_number) }, { key: 'service_name', label: 'Xizmat' },
  ...
  async function load() {
    ...
    agingBox.replaceChildren(card('Debitorlik yoshi (aging)', hBarChart({ items: ag.buckets.map((b) => ({ label: b.label, value: b.amount, sub: b.count + ' ta', color: b.bucket === 'CURRENT' ? 'var(--s1)' : b.bucket === 'NO_DUE' ? 'var(--muted-2)' : b.bucket === '0-7' ? 'var(--warn)' : b.bucket === '8-15' ? 'var(--orange)' : 'var(--crit)' })) })));
