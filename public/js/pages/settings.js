...
export default async function render(root, { setTitle, can, params, me, roleLabel }) {
  ...
  async function load() {
    if (tab === 'rules') {
      ...
      body.replaceChildren(alert('info', `Muhit: Node ${s.env.node} · AI ${s.env.ai_llm ? 'ulangan (' + s.env.ai_model + ')' : 'ulanmagan (qoidalar rejimi)'} · Telegram ${tgBots} · Elektron pochta ${s.env.email ? 'ulangan' : 'ulanmagan'}`),
