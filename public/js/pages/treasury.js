...
import { badge, h, kpiCard, card, fmt, money, short, date, dataTable, kv, formModal, toast, err, today, alert, icon } from '../ui.js';
  ...
  async function load() {
    ...
    box.replaceChildren(
      ...
      h('div', { class: 'grid g3 mb16 tr-row' },
        ...
        card('Hisoblar', h('table', { class: 'tbl' }, h('thead', {}, h('tr', {}, h('th', {}, 'Hisob'), h('th', {}, 'Raqam'), h('th', { class: 'right' }, 'Qoldiq'), h('th', {}))), h('tbody', {}, ...t.accounts.bank.map((a) => h('tr', {}, h('td', {}, a.bank_name, h('div', { class: 'xs muted' }, `boshlang‘ich qoldiq ${fmt(a.opening_balance)}${a.opening_date ? ' · ' + date(a.opening_date) : ''}`), a.adjusted ? h('div', { class: 'xs mt4', title: 'Excel’da boshlang‘ich qoldiq yo‘q — minusni yopish uchun alohida texnik tuzatma. Haqiqiy qoldiq kiritilgach bekor qilinadi.' }, badge('WARNING', 'texnik tuzatma +' + fmt(a.adjustment))) : null), h('td', { class: 'xs muted' }, a.account_number || '--'), h('td', { class: 'right tnum' }, fmt(a.balance)), h('td', { class: 'right' }, can('treasury', 'CREATE') ? h('button', { class: 'btn xs ghost', title: 'Tahrirlash', onClick: () => editAcc('bank', a) }, icon('edit', 13)) : null))), ...t.accounts.cash.map((a) => h('tr', {}, h('td', {}, a.name, h('div', { class: 'xs muted' }, `boshlang‘ich qoldiq ${fmt(a.opening_balance)}${a.opening_date ? ' · ' + date(a.opening_date) : ''}`), a.adjusted ? h('div', { class: 'xs mt4', title: 'Excel’da boshlang‘ich qoldiq yo‘q — minusni yopish uchun alohida texnik tuzatma. Haqiqiy qoldiq kiritilgach bekor qilinadi.' }, badge('WARNING', 'texnik tuzatma +' + fmt(a.adjustment))) : null), h('td', { class: 'xs muted' }, 'kassa'), h('td', { class: 'right tnum' }, fmt(a.balance)), h('td', { class: 'right' }, can('treasury', 'CREATE') ? h('button', { class: 'btn xs ghost', title: 'Tahrirlash', onClick: () => editAcc('cash', a) }, icon('edit', 13)) : null))), h('tr', { class: 'total' }, h('td', { colspan: 2 }, 'Jami'), h('td', { class: 'right' }, fmt(t.total_cash)), h('td', {})))), can('treasury', 'CREATE') ? [h('button', { class: 'btn xs ghost', onClick: () => cashAccForm() }, icon('plus', 13), 'Kassa')] : null, { tight: true })),
