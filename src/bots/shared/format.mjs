...
export const signed = (n) => `${Number(n) > 0 ? '+' : Number(n) < 0 ? '−' : ''}${money(Math.abs(Number(n) || 0))}`;
...
export const GROUP_LABEL = { DIRECT: 'To‘g‘ridan-to‘g‘ri xarajatlar', PAYROLL: 'Oylik (xodimlar)', MARKETING: 'Marketing', ADMIN: 'Ma’muriy', IT: 'IT va aloqa', OFFICE: 'Ofis xarajatlari', OTHER_OPEX: 'Boshqa operatsion', TAX: 'Soliqlar', OTHER: 'Boshqa' };
...
/**
 * Summa: "12 500 000", "12500000", "12,5 mln", "1.2 mlrd", "500 ming", "500k", "12 mln so'm" → son yoki null
 */
export function parseAmount(text) {
  ...
  let value;
  if (mult > 1) {
    const n = raw.replace(/\s/g, '').replace(',', '.'); // "12,5 mln", "1.2 mlrd"
    if (!/^\d+(\.\d+)?$/.test(n)) return null;
    value = Number(n) * mult;
  } else if (/^\d{1,3}([ .,]\d{3})+$/.test(raw)) {
    value = Number(raw.replace(/[ .,]/g, '')); // "12 500 000", "12,500,000", "12.500.000"
  } else {
