...
export function register(app) {
  ...
  const svc = {
    ...
    cashFlow(q = {}) {
      ...
      const IGN = { DIVIDEND: 'Dividend (ta’sischilarga)', TRANSFER: 'Ichki o‘tkazma (bank → kassa)', LOAN: 'Qarz / kredit (заём)', REFUND: 'Qaytarilgan mablag‘', INTEREST: 'Bank foizlari', OTHER_INCOME: 'Boshqa kirim', OTHER: 'Boshqa kirim', REVERSAL: 'Tuzatish yozuvi', PAYROLL: 'Oylik', NON_CONTRACT: 'Shartnomasiz kirim' };
      const inflowDetail = db.all(`SELECT name, SUM(amount) amount FROM (
          SELECT CASE WHEN matching_status='MATCHED' AND matched_contract_id IS NOT NULL THEN 'Mijoz to‘lovlari' WHEN matching_status='IGNORED' THEN COALESCE(ignore_reason,'OTHER') ELSE 'Bog‘lanmagan kirim' END AS name, amount FROM bank_transactions WHERE reversed_at IS NULL AND direction='INCOME' AND COALESCE(cf_class,'OPERATING')<>'TRANSFER' AND tx_date BETWEEN ? AND ?
          UNION ALL SELECT CASE WHEN contract_id IS NOT NULL THEN 'Mijoz to‘lovlari' ELSE 'Kassaga boshqa kirim' END, amount FROM cash_transactions WHERE reversed_at IS NULL AND direction='INCOME' AND COALESCE(cf_class,'OPERATING')<>'TRANSFER' AND tx_date BETWEEN ? AND ?
      ...
      const outflowDetail = db.all(`SELECT name, SUM(amount) amount FROM (
          SELECT COALESCE(ec.name, CASE WHEN t.matching_status='IGNORED' THEN COALESCE(t.ignore_reason,'OTHER') ELSE 'Bog‘lanmagan chiqim' END) AS name, t.amount FROM bank_transactions t LEFT JOIN expenses e ON e.id=t.matched_expense_id LEFT JOIN expense_categories ec ON ec.id=e.category_id WHERE t.reversed_at IS NULL AND t.direction='EXPENSE' AND COALESCE(t.cf_class,'OPERATING')<>'TRANSFER' AND t.tx_date BETWEEN ? AND ?
          UNION ALL SELECT COALESCE(ec.name, CASE k.cf_class WHEN 'FINANCING' THEN 'Kassadan moliyaviy chiqim (dividend / qarz)' ELSE 'Kassadan boshqa chiqim' END), k.amount FROM cash_transactions k LEFT JOIN expenses e ON e.id=k.expense_id LEFT JOIN expense_categories ec ON ec.id=e.category_id WHERE k.reversed_at IS NULL AND k.direction='EXPENSE' AND COALESCE(k.cf_class,'OPERATING')<>'TRANSFER' AND k.tx_date BETWEEN ? AND ?
        ) GROUP BY name ORDER BY amount DESC`, p.from, p.to, p.from, p.to).map((x) => ({ ...x, name: IGN[x.name] || x.name }));
    ...
    dashboard(range = null) {
      ...
      return {
        ...
        data_span: db.get("SELECT MIN(d) first, MAX(d) last FROM (SELECT tx_date d FROM bank_transactions WHERE reversed_at IS NULL AND tx_date>='2000-01-01' UNION ALL SELECT tx_date FROM cash_transactions WHERE reversed_at IS NULL AND tx_date>='2000-01-01' UNION ALL SELECT recognized_at FROM revenue_recognition WHERE status='RECOGNIZED')"),
