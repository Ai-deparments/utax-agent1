...
export function register(app) {
  ...
  const svc = {
    ...
    dataQuality() {
      ...
      push('BALANCE_ADJUSTED', 'WARNING', 'Qoldiqda texnik tuzatma bor — haqiqiy boshlang‘ich qoldiq kiritilmagan', db.all("SELECT j.id, j.account_type, j.amount, COALESCE(ba.bank_name, ca.name) AS name FROM balance_adjustments j LEFT JOIN bank_accounts ba ON j.account_type='BANK' AND ba.id=j.account_id LEFT JOIN cash_accounts ca ON j.account_type='CASH' AND ca.id=j.account_id WHERE j.reversed_at IS NULL").map((j) => ({ entity: j.account_type === 'BANK' ? 'bank_account' : 'cash_account', id: j.id, label: `${j.name}: +${Number(j.amount).toLocaleString('ru-RU')}` })));
